...
int main() {
    ...
    auto main_renderer = Renderer([&] {
        return vbox({
            text("Main UI") | bold,
            text("Press 'n' to add notification"),
            }) | border;
        });
    ...
    auto component = CatchEvent(container, [&](Event event) {
        if (event == Event::Character('n')) {
            notifications.push_back("Hello Notification!");
...
//        return vbox({
//            text("文件浏览器主界面") | center,
//            text("按 'n' 触发一个通知，按 'q' 退出。") | center
//            }) | border;
//        });
//
//    // 构建带通知的顶层渲染器
...
//        if (show_notify) {
//            // 构建通知 UI
//            Element toast = text(notify_text)
//                | bold
//                | color(Color::Black)
...
//
//            // 利用 filler() 将通知推到右下角
//            Element toast_layer = vbox({
//                filler(), // 把下方的 hbox 挤到最底部
//                hbox({
//                    filler(), // 把右侧的 toast 挤到最右边
//                    toast
//                })
//                });
//
