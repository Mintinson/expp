 ...
 * to basic text or binary stream classifications based on the `core::filesystem::is_previewable`
 * heuristic.
 *
...
[[nodiscard]] std::string guess_mime_from_extension(const fs::path& path) {
    ...
    if (const auto it =
            std::ranges::find(kMimeMap, extension, &decltype(kMimeMap)::value_type::first);
        it != kMimeMap.end()) {
        return std::string{it->second};
    }

    // TODO: For better accuracy, consider using libmagic for content sniffing, but that would require
 ...
 *
 * @warning This function performs blocking disk I/O and should not be executed directly on the
 * UI/IO threads.
 *
...
[[nodiscard]] core::Result<MimePayload> detect_mime_blocking(const MimeRequest& request) {
    ...
    auto mime = guess_mime_from_extension(request.target);
    MimePayload payload{
        .mimeType = mime,
        .previewable = mime.starts_with("text/"),
    };
    ...
    if (core::global_config().config().analysis.mimeSniffing) {
        ...
        if (magic != nullptr) {
            if (magic_load(magic, nullptr) == 0) {
                if (const char* detected = magic_file(magic, request.target.string().c_str());
                    detected != nullptr) {
...
public:
    ...
    [[nodiscard]] core::Task<core::Result<PreviewPayload>> loadPreview(
        ...
        auto preview_result = co_await core::invoke_on(runtime_->diskExecutor(), [request, &lines] {
            if (request.cancellation.isCancellationRequested()) {
        ...
        PreviewPayload payload{
            ...
            .previewable = preview_result.value(),
        };
